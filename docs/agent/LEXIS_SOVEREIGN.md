...
2. **Voice Calibration Pass:** The synthesis agent identifies rhetorical patterns, preferred sentence cadences, and high-conviction vocabulary clusters. This data populates `voice_calibration_matrix.yaml`.
...
**Context:** This pass uses an execution kernel optimal for deterministic DAG construction with `DCCDSchemaGuard(schema="CHAPTER_MANIFEST", enforcement="strict")` active. Manifold α (Voice) is disabled. No prose is generated here.
...
5. For each section, generate an Argument Blueprint (3-5 bullet points of logical moves, not prose) and tag all factual claims with `[EVIDENCE_REQUIRED]` or `[FOUNDER_ATTESTATION]`.
...
**Context:** This pass uses a constitutional synthesis kernel with `ContextLock(anchor="VOICE_CALIBRATION_MATRIX", refresh_interval=2048)` active. Manifold β (structure) is locked. No structural changes are permitted here.
...
**Context:** A dedicated Critic agent (adversarial evaluation mode) evaluates the chapter draft against all five Epistemic Matrix dimensions.
...
When multiple models collaborate, they are never permitted to evaluate each other's outputs directly. Each model is assigned a strict Epistemic Jurisdiction Manifest (EJM): e.g., one model evaluates against the VCM; another evaluates against the DCCD schema. Cross-evaluation is architecturally blocked. This prevents models from forming a false consensus on generic text by ensuring each is measuring against a different, founder-anchored standard.
...
1. **Metric 1 — Voice Match Score (VMS) ≥ 0.78 (Primary Voice Fidelity)**
   ...
   - *Measurement:* Computed per section via an embedding model after each WRITE pass.
...
2. **Metric 2 — CFDI ≤ 0.15 (Confidence-Fidelity Divergence Index)**
...
3. **Metric 3 — Autonymic Bypass Compliance < 1% (Jargon-Elimination Rate)**
...
4. **Metric 4 — Semantic Density Index ≥ 0.12 entities/token (Intellectual Density)**
...
5. **Metric 5 — Flesch Reading Ease 45–65 (Register Calibration)**
