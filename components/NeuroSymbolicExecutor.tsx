  ...
  return (
    <div className="p-8 max-w-5xl mx-auto h-full overflow-y-auto">
      <div className="mb-8 flex justify-between items-start">
        <div>
          <h2 className="text-display-lg font-mono font-bold text-primary mb-2">Neuro-Symbolic Executor</h2>
          <p className="text-on-surface-muted font-mono text-sm">Hybrid procedural grounding for complex mathematical operations.</p>
        </div>
        {successRate !== null && (
          <div className="bg-surface-raised border border-border px-4 py-2 rounded-none flex flex-col items-end ">
            <span className="text-[10px] font-mono text-on-surface-muted uppercase tracking-wider mb-1">Procedural Success Rate</span>
            <div className="flex items-center gap-2">
              <Activity className="w-4 h-4 text-primary" />
              <span className={`text-xl font-mono font-bold ${successRate >= 80 ? 'text-primary' : successRate >= 50 ? 'text-primary' : 'text-red-400'}`}>
      ...
      <div className="bg-surface-raised border border-border rounded-none p-6 mb-8 ">
        <div className="flex flex-col gap-4">
          <div className="flex-1">
            <label htmlFor="neuro-input" className="block text-xs font-mono text-on-surface-muted uppercase tracking-wider mb-2">Mathematical Problem / Expression</label>
            <textarea
              id="neuro-input"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="e.g., integrate(x^2, x) or Calculate the derivative of sin(x)*e^x"
              className="w-full h-32 bg-surface border border-border rounded-none px-4 py-4 text-on-surface font-mono focus:outline-none focus:border-primary transition-colors resize-none"
              onKeyDown={(e) => {
            ...
            <div className="flex justify-between items-center mt-2">
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="text-xs font-mono text-primary hover:text-on-surface flex items-center gap-1 transition-colors disabled:opacity-50"
      ...
      {trace.length > 0 && (
        <div className="bg-surface border border-border rounded-none overflow-hidden shadow-2xl">
          <div className="bg-surface-raised px-6 py-4 border-b border-border flex items-center gap-3">
            ...
            <h3 className="text-sm font-mono text-on-surface uppercase tracking-wider">Execution Trace</h3>
          </div>
          
          <div className="p-6 space-y-6">
            {trace.map((step, idx) => (
              <div key={idx} className={`flex gap-4 ${step.type === 'final_result' ? 'mt-8 pt-6 border-t border-border' : ''}`}>
                <div className="flex-shrink-0 mt-1">
                  {step.type === 'direct' && <Calculator className="w-5 h-5 text-on-surface-muted" />}
                  {step.type === 'llm_reasoning' && <BrainCircuit className="w-5 h-5 text-primary" />}
                  {step.type === 'tool_call' && <Wrench className="w-5 h-5 text-primary" />}
                  {step.type === 'final_result' && <CheckCircle2 className="w-6 h-6 text-primary" />}
                  {step.type === 'error' && <XCircle className="w-5 h-5 text-red-400" />}
                </div>
                
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    <span className="text-xs font-mono text-on-surface-muted uppercase tracking-wider">
                      {step.type.replace('_', ' ')}
                    </span>
                    {step.status === 'success' && <span className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-mono rounded border border-primary/20">SUCCESS</span>}
                  ...
                  {step.type === 'final_result' ? (
                    <div className="mt-4">
                      <div className="prose prose-invert max-w-none prose-pre:bg-surface prose-pre:border prose-pre:border-border">
                      ...
                      <div className="mt-4 flex justify-end">
                        <button
                          ...
                          className="px-4 py-2 bg-[#2a2a2a] hover:bg-[#333] text-on-surface text-xs font-mono rounded-none flex items-center gap-2 transition-colors border border-[#444]"
                    ...
                    <div className="text-sm text-on-surface font-mono leading-relaxed">
                      {step.content}
                    </div>
                  )}
                  
                  {step.details && (
                    <div className="mt-2 p-3 bg-surface border border-border rounded-none text-xs font-mono text-on-surface-muted overflow-x-auto">
                      {step.details}
                    </div>
                  )}
                </div>
            ...
            {loading && trace[trace.length - 1]?.type !== 'error' && trace[trace.length - 1]?.type !== 'final_result' && (
              <div className="flex gap-4 items-center pl-1">
                ...
                <span className="text-sm font-mono text-on-surface-muted">Processing...</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {savedResults.map((saved, idx) => (
              <div key={idx} className="bg-surface-raised border border-border rounded-none p-4 shadow-sm relative group">
                <button 
                ...
                <div className="mb-2">
                  <span className="text-xs font-mono text-on-surface-muted uppercase tracking-wider">Query</span>
                  <p className="text-sm font-mono text-on-surface mt-1 line-clamp-2">{saved.query}</p>
                </div>
                <div>
                  <span className="text-xs font-mono text-on-surface-muted uppercase tracking-wider">Result</span>
